...
describe('listMessages static api for Message model', () => {
  before((done) => {
    utils.setUpDbBeforeTest(done);
  });

  beforeEach(utils.createMessages.bind(this));

  it('Messages returned correctly', async () => {
    const messages = await Message.listMessages(this.chat.id, 2);
  ...
  it('First page of messages returned if no page provided', async () => {
    const messages = await Message.listMessages(this.chat.id);
    // Get texts from messages
    const texts = messages.map(message => message.text);
    // Verify that messages returned in correct order
    const expectedTexts = [
...
describe('listMessagesWithAuthor static api for Message model', () => {
  before((done) => {
    utils.setUpDbBeforeTest(done);
  });
  ...
  it('Messages populated with extra data correctly', async () => {
    const messages = await Message.listMessagesWithAuthor(this.chat.id);
